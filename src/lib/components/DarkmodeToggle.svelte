...
<button bind:this={toggleButton} onclick={toggle} class="theme-toggle group flex items-center gap-2 text-amber-600 dark:text-amber-300 outline-none {className}" class:theme-toggle--toggled={enabled} type="button" title="Toggle theme" aria-label="Toggle theme">
    <!-- https://codepen.io/cssence/pen/RRNvQX -->
    <!-- group-data-[state=enabled]:text-amber-600 group-data-[state=enabled]:dark:text-amber-300 -->
    ...
    <span class="text-slate-800 dark:text-white group-hover:text-tuscany-600 group-focus:text-tuscany-600 dark:group-hover:text-tuscany-400 dark:group-focus:text-tuscany-400 md:sr-only">Switch to {theme === 'dark' ? 'light' : 'dark'} mode</span>
</button>
